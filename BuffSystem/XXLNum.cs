...
public struct XXLNum
{
    ...
    public static void LocalizePrefix(string[] prefixes)
    {
        _prefixes = prefixes;
    }
    public static void LocalizePrefix()
    {
        _prefixes = new string[] { "K", "m", "M" };
    }
    [JsonProperty("Mantice")]
    [DefaultValue(0)]
    private float mantice;
    [JsonProperty("TenPower")]
    [DefaultValue(0)]
    private int ten_power;
    ...
    public override string ToString()
    {
        if (ten_power > 0)
        {
            int power = ten_power / 3 - 1;
            return mantice.ToString("0.0") + " " + _prefixes[power];
        }
        else if (ten_power == 0)
            return mantice.ToString("0");
        else
            return (mantice * Mathf.Pow(10, ten_power)).ToString();
    }
    ...
    {
        return ten_power.GetHashCode() + mantice.GetHashCode();
    }
}
