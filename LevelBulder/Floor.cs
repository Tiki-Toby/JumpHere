...
namespace Assets.Scripts.Level
{
    public abstract class AbstractFloor
    {
        ...
        {
            return InitFloor(trackPanel, Center + Sizes.NablaMultiply(direction + Vector3.up) + direction * jumpDistance, direction);
        }
        ...
        private AbstractFloor InitFloor(Transform floorT, Vector3 position, Vector3 direction)
        {
            ...
            AbstractFloor floor = InitFloor(floorT);
            floorT.position = position + floor.Sizes.x * direction - floor.Center;
            floor.InitParams();
            return floor;
        }
        public void ReplaceFloorBySize(Transform floorT)
        {
            floorT.position = Center;
            //floorT.localScale = new Vector3(floorT.localScale.x / );
            GameObject.Destroy(floorObject.gameObject);
            floorObject = floorT;
        }
        public void AddObjectOnFloorCenter(Transform obstacle)
        {
            AddObjectOnFloorCenter(obstacle, Vector3.up * (Sizes.y + 0.2f));
        }
        public void AddObjectOnFloorCenter(Transform obstacle, Vector3 position)
        {
        ...
        public void AddObjectOnFloorWithOffetCenter(Transform obstacle, Vector3 sizeInflunce, Vector3 offset)
        {
            obstacle.parent = floorObject;
            obstacle.localPosition = Sizes.NablaMultiply(sizeInflunce) + offset;
            obstacle.localRotation = Quaternion.Euler(0, 90, 0);
            //if (direction.x == 0)
            //{
            //    obstacle.localPosition = Sizes.NablaMultiply(Vector3.right) + offset;
            //    obstacle.localRotation = Quaternion.Euler(0, 90, 0);
            //}
            //else
            //{
            //    obstacle.localPosition = Sizes.NablaMultiply(Vector3.forward) + offset;
            //    obstacle.localRotation = Quaternion.Euler(0, -90, 0);
            //}
        }
